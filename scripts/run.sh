...
docker run -d                       -t                            bewt85/etcd_registrar register backend  0.0.1 $HOST_IP 6000 --peer $HOST_IP:9000
docker run -d --name frontend_0.0.1 -t -p 5000:8080 --dns $DNS_IP bewt85/frontend:0.0.1
docker run -d                       -t                            bewt85/etcd_registrar register frontend 0.0.1 $HOST_IP 5000 --peer $HOST_IP:9000
docker run -d --name backend_0.0.2  -t -p 6001:8080 --dns $DNS_IP bewt85/backend:0.0.2
docker run -d                       -t                            bewt85/etcd_registrar register backend  0.0.2 $HOST_IP 6001 --peer $HOST_IP:9000
docker run -d --name frontend_0.0.2 -t -p 5001:8080 --dns $DNS_IP bewt85/frontend:0.0.2
docker run -d                       -t                            bewt85/etcd_registrar register frontend 0.0.2 $HOST_IP 5001 --peer $HOST_IP:9000
